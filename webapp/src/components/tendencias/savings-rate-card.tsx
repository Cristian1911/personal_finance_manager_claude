...
const config = {
  rate: { label: "Tasa de ahorro", color: "var(--z-brass-hot)" },
  target: { label: "Meta", color: "var(--z-income)" },
  ...
  if (savings.length === 0) {
    return (
      <div className={`${PANEL_SURFACE_CLASS} p-6 text-center text-sm text-z-sage-dark`}>
        Sin datos de ahorro en el periodo.
      </div>
    );
  }
  ...
  return (
    <div className={`${PANEL_SURFACE_CLASS} p-4`}>
      ...
      <ChartContainer config={config} className="h-28 w-full">
        <AreaChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: 0 }} accessibilityLayer>
          <defs>
          ...
          <ReferenceLine y={20} stroke="var(--color-target)" strokeDasharray="3 3" />
