...
**Decisiones del usuario (2026-09-15):** Periodo y ¿Comprarlo? aparcados; mecanismo = Más → "Herramientas avanzadas" colapsado; guía = coach-marks secuenciados, 2 por página, sin overlay; sin tour modal (junio). Deudas personales: solo se aparca la entrada de nav, los tiles contextuales del movimiento se quedan (confirmado por el usuario). Destinatarios no se renombra. Más regrupado en 4 grupos (Cuentas y saldos · Entender · Planificar · Sistema).

**Fase B mergeada el 2026-09-15:** #403 #404 #405 #406 (+ docs #407). **Fase A en curso:** slice 5 en PR #408 (ver ítem 5). Quedan los slices 6 a 9. Detalle de lo hecho, por si hay que revisitarlo:

1. **(P0) B1a `feat/recorte-nav`** — `mobile-link-grid.tsx`: 4 grupos con 7 tiles — "Cuentas y saldos" (Cuentas, Importar) · "Entender" (Destinatarios, Tendencias; sustituye a ORGANIZAR/ANALISIS) · "Planificar" (Recurrentes, luego Plan|Deudas por `nav_focus`) · "Sistema" (Ajustes) — + `ADVANCED_GROUP` en orden (Categorizar, Categorías, Etiquetas, Modos, Periodo `/plan?tab=periodo` icono CalendarRange, Deseos, ¿Comprarlo?, Deudas personales) en `<Collapsible defaultOpen={false}>` con eyebrow "Herramientas avanzadas". `navigation.ts`: `WORKSPACE_NAV` = Destinatarios, Importar, Cuentas, Deudas; nuevo `ADVANCED_NAV` = Categorizar, Deudas personales, Modos; `sidebar.tsx:51-70` y `layout/mobile-nav.tsx:64-83` lo pintan colapsado. `plan-tab-nav.tsx`: `nav?: false` en periodo/deseos y filtro en el `<nav>` (NO quitar del array: `PlanTab` se deriva de `PLAN_TABS` :15; `VALID_TABS`, la lista inline de `usePlanTab` :49 y `MOBILE_TAB_TITLES` quedan en 5). `plan-drill-cards.tsx:88-93,100-105` quitar chips Periodo/Deseos; `plan/page.tsx:51-58` quitar fetch de wishlist si solo alimentaba el chip. `widgets-zone.tsx:35-40` quitar `DeseosWidget` + fetches `:17-18`. `timeline-model.ts:122` CTA → `/plan?tab=recurrentes`. Borrar `inicio-discovery-rail.tsx`, `plan-decision-rail.tsx`, `burndown-expandable.tsx` (0 imports). `patchGuided` (`guided-experience.ts:157`): seed `getDefaultConfig(app_purpose)` cuando `dashboard_config` es NULL. Gates `zetas-front-guy`, `perf-auditor`. **Primero: B2 depende del seed.**
2. **(P0) B1b `feat/filtro-sin-categoria`** — `validators/transaction.ts:88` `categoryId: z.union([uuidStr(), z.literal("none")]).optional()`; `actions/transactions.ts:521` `"none"` → `.is("category_id", null)`; chip en `transaction-filters.tsx`; `transactions/page.tsx:231` AttentionCard href → `/transactions?categoryId=none`; quitar panel "Compra consciente" `:243-263`. Gate `server-action-reviewer`.
3. **(P0) B1c `feat/suscripciones-en-recurrentes`** — `plan-tab-recurrentes.tsx`: `getSubscriptions()`, `<SubscriptionSuggestions>` (existe, props `{suggestions, currency}`) arriba de la lista con "Ver todas" → `/suscripciones`; `Set<recurring_template_id>` → prop `subscriptionTemplateIds` a `recurring-list.tsx`, `<Badge variant="outline">Suscripción</Badge>` junto a la frecuencia (:233, precedente "Pausada" :251-255); quitar botón "Suscripciones ›" (:111) y footer del empty state (`recurring-list.tsx:65-70`). Móvil web `mobile-recurrentes-view.tsx` (:322-333, :387-400): **antes revisar de dónde saca filas el hook (`hook.isHydrated` :322) y de dónde saldría el set de suscripciones**. `packages/shared/dashboard-layout.ts:66` quitar `puedo_comprarlo` de `DEFAULT_LAYOUT`; `lib/dashboard/widgets.ts:35` `available: false`. Gates `mobile-webapp-parity`, `cache-doctor`, `server-action-reviewer`.
4. **(P0) B2 `feat/guia-de-pagina`** (tras mergear B1a) — `coach-mark.tsx`: `useCoachMarkSequence(ids)` (~15 líneas sobre el cache de módulo) + prop `step?: string` como eyebrow. 6 marcas: `import-upload` (dropzone `step-upload.tsx:522`), `import-password` (`:639`, solo ruta PDF → sin "N de 2"), `budget-builder-sum` (`budget-builder.tsx:323`), `budget-builder-add` (`:277`), `planner-steps` (`scenario-planner.tsx:215`), `planner-compare` (`debt-simulator.tsx:496`). Copy en spec §6. Gates `zetas-front-guy`, `perf-auditor`.
5. **(P0 · HECHO, PR #408)** `detectRecurringCandidates` en `@zeta/shared` (`utils/recurring-candidates.ts` + 16 tests): ≥2 cobros, agrupa por destinatario o `cleanDescription` + moneda, cadencia mensual (gap 28–34 o mismo día ±3), banda de monto ±10 % de la mediana, excluye `flow_class_effective` ∈ DEBT_*/SELF_TRANSFER/CASH_WITHDRAWAL (el guard NO es el regex de `auto-categorize`: `flow_class` ya se persiste). Webapp: `actions/recurring-candidates.ts` (caché `transactions`+`recurring`, 100 días, excluye `recurrence_group_id`/plantilla activa por destinatario), `RecurringCandidateCallout` en el detalle de tx con "Programar" (reusa `PromoteToRecurringButton`) y "No es recurrente" persistido en `guidedExperience.discoveries` (§4.2, id `recurring:<key>`; `dismissDiscovery`/`getDismissedDiscoveryIds` listos para el slice 6). `lib/subscriptions/detect.ts`: núcleo agnóstico del cliente + `scheduleSubscriptionDetection` en `after()` tras quick capture, formulario, aprobación de correo, Telegram, `/api/capture` y webhook de correo. **Follow-ups:** (a) refactorizar `detectSubscriptions` sobre el detector nuevo; (b) chip "Parece recurrente" en RN; (c) si el slice 6 necesita correr el detector por admin, usar `clean_description_hmac` como clave (cifrado → NULL vía admin); (d) plantillas creadas desde cero para un comercio sin destinatario no se cruzan por nombre: el grupo cae cuando cualquier fila suya queda vinculada (`recurrence_group_id`); (e) `subscription-detector.ts` tampoco excluye `installment_group_id` (el detector nuevo sí, en la query); (f) el webhook de Telegram no asigna destinatario, así que no dispara detección: al añadirle `matchTransactionToDestinatario`, enganchar `scheduleSubscriptionDetection` como en `/api/capture`; (h) `dismissDiscovery` devuelve éxito sin escribir cuando ya hay 300 claves y el id es nuevo: el chip se oculta optimista y vuelve al recargar (teórico hoy; si el slice 6 acumula descubrimientos, podar `discoveries` o devolver error); (i) el último commit de #408 (grupos con fila vinculada se quedan en la entrada del detector) es posterior a la review de `recurring-doctor`: lógica conservadora (solo puede suprimir chips), pero sin segunda pasada; (g) el slice 7 debe migrar `RecurringCandidateCallout` a `DiscoveryInline` sobre `ui/alert.tsx` (variante brass) y extraer `PANEL_INSET_BRASS_CLASS` (`border-z-brass/20 bg-z-brass/8`, hoy repetido 3 veces en styles.ts + callout).
6. **(P0)** `getDiscoveries()` en `actions/discoveries.ts` (federa detectores existentes + el nuevo; máx. 2 visibles) + tipos/zod `discoveries` en `guidedExperience` + `PrimerosPasos` con sección "Zeta notó" (la tarjeta no muere al completar el checklist, `guided-experience.ts:131`). Gates `server-action-reviewer`, `cache-doctor`, `perf-auditor`.
7. **(P1)** `DiscoveryInline` (envuelve `ui/alert.tsx`, shadcn stock sin tokens) en detalle de tx, `/plan?tab=recurrentes|presupuesto`, `/tendencias`, `/destinatarios`, `/deudas`. Mismo id que la tarjeta; descartar en uno apaga los dos.
8. **(P1)** Digest semanal in-app: tarjeta lunes en Inicio con `getWeeklyDigest` (hoy cero callers); copy del engine (mapas de emoji) por la regla de marca; descarte hasta el lunes siguiente.
9. **(P1)** D1 chooser "¿Cómo quieres empezar?" reemplaza el paso de estimaciones del onboarding (`app/onboarding/page.tsx:457` exige ingreso + gasto). Mock: `ds-guided/preview/onboarding-chooser.html`.
- **(P2, móvil RN)** Espejar recorte de nav y capa guiada una vez `guidedExperience` esté sincronizado. Gates parity/sync.
- ~~**(P3)** Detector de Modos: ≥5 tx en otra moneda o etiqueta de viaje en ≤10 días → "¿Agrupar como viaje?".~~ Cubierto (2026-09-16) por la bandeja "¿Fueron del viaje?" de cada viaje (`getModoCandidates`, moneda extranjera premarcada). Queda como refinamiento: sugerir crear un viaje desde Inicio cuando haya ≥5 gastos en otra moneda en ≤10 días sin viaje que los cubra.

**Hallazgos sueltos (2026-09-15):** el mismatch `tagId`/`tags` de Movimientos quedó corregido en #404. Pendiente de verificación manual (los agentes de review se cortaron por el límite de sesión en #404 y #405, y ninguno de los cuatro PRs tuvo recorrido autenticado en navegador): Más a 390px con "Herramientas avanzadas" cerrada y abierta; sidebar desktop con la sección colapsada abriéndose sola en `/modos`; `/transactions?categoryId=none` con el Select mostrando "Sin categoría"; Recurrentes con una sugerencia viva; primera visita a `/import`, `/presupuesto/armar` y `/deudas/planificador` mostrando la primera marca y la segunda tras "Entendido". Migrar los cuatro eyebrows del sidebar ("Principal", "Herramientas", "Herramientas avanzadas", "Sistema") a `SECTION_EYEBROW_CLASS` en una pasada (hoy usan `text-[11px]`, `zetas-front-guy` #403).

**Deuda de diseño detectada:** `claude-ai-design/ds-guided/colors_and_type.css` y la copia en el proyecto Claude Design tenían los tokens pre-#315 (brass `#937844`, ink `#121412`). Parchados a `#a98a51`/`#0b0b0c`/superficies neutras esta sesión y subidos. Cualquier preview nuevo debe arrancar de `claude-ai-design/primeras-semanas/colors_and_type.css`. Residuo: `--z-surface-1` no existe en `globals.css` (era el azul-negro `#131720`); lo usa solo `ds-guided/preview/info-hint.html`, así que quedó remapeado a `#1c1c1f`. En el próximo sync, migrar ese preview a `--z-surface-2` y borrar el token.
...
- ~~**(P2) Editar modo no está cableado en UI.**~~ Cableado (2026-09-16): `/modos/[id]/edit` reutiliza `ModoWizard` con `initial` + `initialParticipants`; `ModoFormDialog` se eliminó.
...
- **(P1 — mobile parity) `modos`/`modo_participants`/`modo_tx_reviews` son webapp-only.** Correr `mobile-webapp-parity` + `mobile-sync-doctor` antes de cualquier trabajo de modos en móvil (enum/columna drift, las tablas nuevas no están en SQLite). El hook de viaje activo (`applyActiveModoTag`) tampoco corre para capturas hechas desde la app móvil: esas caen en la bandeja como importadas hasta que móvil lo espeje.

## Importación PDF — enriquecimiento por fila, cuotas compartidas y resumen accionable (2026-09-16) — follow-ups

Shipped: paso Revisar con viaje / compartido con… / etiquetas / nota por fila (panel móvil + popover desktop), chips de triage y barra en lote; compras a cuotas compartidas como compra completa (`estimateInstallmentPlan`, columnas `personal_debts.installment_group_id/installment_total/group_total_amount/interest_amount`, migración `20260916120000`), `recomputeSplitRepaid` multi-fila; pantalla "Listo" con "Qué sigue" y "Omitidas" con motivo.

- **(P1) Fase 2 — bandeja persistente de importación.** Tabla `import_batches` (`id, user_id, capture_method, source_filename, imported, skipped, errors, skipped_rows jsonb, summary jsonb, created_at`) + `transactions_enc.import_batch_id` (proceso de 6 pasos con `supabase-migrator`) estampado en `buildInsertRow`; página `/import/[batch]` con pestañas Sin categoría / Compartidas / Del viaje / Omitidas leyendo `transactions where import_batch_id`; filtro `import_batch_id` en `/transactions`; señal en `actions/attention.ts` ("Último extracto: N sin categoría", `AttentionPage` += `"import"`). `insertedByKey` en `importTransactions` ya tiene los ids creados; exponerlos solo cuando exista el batch (hoy se descartan para no inflar la respuesta).
- **(P2) Transferencias propias al importar.** `matchOwnAccount` ya clasifica el `flow_class` pero nunca escribe `transfer_group_id`: emparejar OUTFLOW/INFLOW entre cuentas propias dentro del lote (o contra un movimiento existente) desde el paso Revisar.
- **(P2) `is_excluded` por fila al importar** (importar pero no contar) — descartado en esta entrega por decisión de producto; barato de añadir sobre `RowEnrichment`.
- **(P2) Tasa por fila en el parser.** Los extractos de TC traen tasa por compra; el modelo `ParsedTransaction` no la expone, así que el interés estimado usa la tasa de cabecera. Añadir `interest_rate` por fila en `services/pdf_parser/models.py` y usarla en `share.ea_rate_percent` cuando exista.
- **(P2) Cuotas compartidas antes de importar el resto.** Si el usuario reparte la cuota 3 desde el viaje (`shareModoTransactions`) o desde "Repartir" sin pasar por el import, se reparte como cuota suelta (monto de la cuota) y esa deuda no lleva `installment_group_id`, así que `shareImportedRows` no la ve como "ya compartida": una cuota posterior compartida al importar crearía un grupo de compra completa además de la cuota suelta (doble conteo). Reusar la regla de compra completa también ahí cuando la fila tenga `installment_total > 1`, y hacer que `existingByGroup` también reconozca deudas cuyo `origin_transaction_id` sea una cuota hermana.
- **(P3) `splitExistingTransaction` vive en un módulo `"use server"`.** Next lo publica como endpoint aunque su primer argumento (el cliente) no sobreviva la serialización; moverlo con `SplitTxConfig`/`SplitTxOptions` a `lib/personal-debts/split-transaction.ts` (`server-only`) como ya hacen `ad-hoc.ts` y `membership.ts`.
- **(P3) Reparto que sobrevive a la reconciliación.** Cuando una fila importada absorbe una manual que ya era pago compartido, el reparto de la importada se descarta (bien) pero `split_group_id` y `personal_debts.origin_transaction_id` se quedan en la fila oculta: portar el grupo a la sobreviviente como hace `carryRecurringLinkToSurvivor` (relacionado con el P1 de net-out de más arriba).
- **(P3) Sobrepago antes de que lleguen más cuotas.** `allocateRepaidAcrossRows` deja sin asignar lo que excede las cuotas ya importadas hasta el siguiente import; la tarjeta usa `recovered` derivado de las deudas, así que se ve bien, pero el gasto efectivo del mes se corrige con retraso.

## Viajes y eventos — viaje activo + bandeja (branch `claude/shared-trip-expenses-ifsfvc`, 2026-09-16) — follow-ups

Shipped: `modos.is_active` (uno por usuario, índice parcial), `auto_tag_id` (etiqueta propia find-or-create bajo el grupo "Viajes y eventos"), tabla `modo_tx_reviews` (decisión incluido/excluido por tx), regla `classifyModoCandidate` en `@zeta/shared`, hook `applyActiveModoTag` tras `linkTransactionToOccurrence` en form / captura rápida / Telegram / API de captura, wizard `/modos/nuevo` de 3 pasos (candidatos del rango), bandeja `/modos/[id]/revisar`, detalle con totales por moneda + reparto por fila, "Viajes y eventos" en Herramientas/Entender, FAB contextual, tarjeta en Tendencias. Migración `20260916030000_modo_activo.sql`.

- **(P2) Señal de ubicación en la bandeja.** `transaction_locations_enc.place_country` (captura solo móvil, cifrada — leer con el cliente cacheado, nunca admin) distinto de Colombia ⇒ candidato fuerte, premarcado como la moneda extranjera. Requiere que móvil esté poblando `location_id`.
- **(P2) Descartar en lote borra silenciosamente.** "Descartar el resto" en la bandeja marca `excluded` para todo lo no marcado; no hay "deshacer" ni forma de volver a ver excluidos. Añadir un filtro "Descartados" en el detalle con "Volver a considerar" (delete de la fila en `modo_tx_reviews`).
- **(P3) Membresía = etiqueta, sin fechas (2026-09-16).** Un movimiento etiquetado cuenta para el viaje aunque esté fuera del rango (vuelos pagados antes). Consecuencia: dos viajes que compartan una etiqueta ven los mismos movimientos etiquetados; si molesta, añadir un aviso en el wizard cuando la etiqueta elegida ya pertenece a otro viaje.
- **(P3) Cambiar la etiqueta propia de un viaje re-etiqueta nada.** Al editar un viaje y elegir otra etiqueta como `auto_tag_id`, las tx ya marcadas con la anterior siguen siendo miembros (la anterior sigue en `tag_ids`), pero las nuevas capturas usan la nueva. Aceptable; documentar en el wizard si confunde.
- **(P3) `listModosWithTotals` trae los candidatos de la unión de rangos** (una query con `limit 500`) para el conteo "por revisar" de todas las tarjetas. Con muchos viajes largos el cap deja conteos parciales; si pasa, mover el conteo a un RPC agregado.
