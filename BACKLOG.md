...
- **Mobile Tendencias — T4 (drill-down + search)**: tap a category / recipient row → expand to the transactions behind it (`getDrilldownTransactions` + a `categoryHierarchy` in the dataset via `buildCategoryHierarchy`), plus a search box on the category + recipient lists. The webapp `CategoryTrendList`/`TopRecipientsCard` have this (parent/child accordion + `DrilldownTransactions` + `foldForSearch`); mobile T1–T3 ship the flat lists only. This is the last Tendencias parity gap.
