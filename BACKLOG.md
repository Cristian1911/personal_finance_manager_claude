...
- **(P2) `docs/design-system/TOKENS.md` no documenta la convención NativeWind.** El patrón `{base}-{percent}` (`z-brass-12`, `z-surface-2-6`, `white-10`) y el hecho de que la sintaxis `/opacity` de la webapp falla en silencio en móvil no están escritos en ninguna parte — que es justo el origen de las 50 clases rotas que se arreglaron en esta rama. Añadir una subsección "Mobile (NativeWind v3)" apuntando a `mobile/tailwind.config.js` como fuente de verdad.
