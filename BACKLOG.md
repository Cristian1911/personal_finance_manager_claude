...
- **[P2] Local `accounts` missing `is_demo`** (found by `mobile-webapp-parity` on Tendencias, branch `feat/mobile-tendencias`). Mobile `accounts` has no `is_demo` column, so webapp demo accounts pull down as regular accounts. Low risk (mobile has no demo mode), but analytics/aggregates can't exclude demo-account data the way the webapp does. Fix: `ALTER TABLE accounts ADD COLUMN is_demo INTEGER NOT NULL DEFAULT 0` (v23) + add `"is_demo"` to `BOOLEAN_FIELDS.accounts` in `pull.ts` + `AND a.is_demo = 0` in `getTendenciasDataset`'s accounts subquery.
- **Mobile recurrentes authoring — follow-ups** (R1 create form, branch `feat/mobile-recurrentes-authoring`; gates clean). Create form ships at parity — `day_of_month`/`day_of_week` null is byte-identical to the webapp (the occurrence generator anchors on `start_date`); debt-INFLOW auto-category applied. Deferred: (a) **R2 edit form** — `/recurrentes/[id]/edit` + an `updateRecurringTemplate` repo fn (the focus-mode regex `^/recurrentes/[^/]+/edit$` is already registered); (b) **subscription toggle / sync** — the webapp `is_subscription` switch calls `upsertSubscriptionFromTemplate` on create AND update, so mobile-created templates never register in the `subscriptions` table, and editing a subscription-linked template from mobile leaves its `subscriptions` row stale (wrong destinatario/currency) — `mobile-webapp-parity` P2 on R2 (feature gap, not data integrity; status never wrongly flipped); (c) **multi-currency `sub_payments`** for debt abonos (mobile repo doesn't handle) — **also a sync column drift** (`mobile-sync-doctor` on R2): `recurring_transaction_templates.sub_payments JSONB` exists on Supabase but not in mobile SQLite, so pulls silently drop it (harmless today; the UPDATE path keeps the remote value via PATCH). When implementing: `ALTER TABLE recurring_transaction_templates ADD COLUMN sub_payments TEXT` + add `recurring_transaction_templates: ["sub_payments"]` to `JSON_FIELDS` in `pull.ts`; (d) extract the duplicated `FieldLabel` helper (capture.tsx + RecurringForm) → `components/ui/FieldLabel.tsx`.
