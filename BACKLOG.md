...
**Fase B implementada el 2026-09-15:** B1a #403 (mergeado), B1b #404, B1c #405, B2 #406 (abiertos). Queda la Fase A (slices 5 a 9). Detalle de lo hecho, por si hay que revisitarlo:
...
5. **(P0)** `detectRecurringCandidates` en `@zeta/shared` (generaliza `subscription-detector.ts`: ≥2 cobros, mismo destinatario o `cleanDescription`, mismo día ±3 o gap 28–34 d, ±10% monto, excluye deuda/transferencias/retiros) + test + chip "Parece recurrente" en detalle de tx + `runSubscriptionDetection` tras captura email/Telegram/quick (`import-transactions.ts:2081`). Gate `recurring-doctor`.
