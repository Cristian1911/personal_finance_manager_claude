...
- **Mobile recurrentes authoring — follow-ups** (R1 create form, branch `feat/mobile-recurrentes-authoring`; gates clean). Create form ships at parity — `day_of_month`/`day_of_week` null is byte-identical to the webapp (the occurrence generator anchors on `start_date`); debt-INFLOW auto-category applied. Deferred: (a) **R2 edit form** — `/recurrentes/[id]/edit` + an `updateRecurringTemplate` repo fn (the focus-mode regex `^/recurrentes/[^/]+/edit$` is already registered); (b) **subscription toggle** — the webapp `is_subscription` switch calls `upsertSubscriptionFromTemplate`, so mobile-created templates never register in the `subscriptions` table (feature gap, not data integrity); (c) **multi-currency `sub_payments`** for debt abonos (mobile repo doesn't handle); (d) extract the duplicated `FieldLabel` helper (capture.tsx + RecurringForm) → `components/ui/FieldLabel.tsx`.
