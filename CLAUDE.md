...
- **Import-time enrichment (PDF)**: the review step sends per-row `tag_ids`, `modo_id`, `share`, `notes` in `TransactionToImport`; `importTransactions` applies them after insert + reconciliation (§9), never failing the import on an enrichment error. `ImportResult` returns `skippedRows` (reason + existing id), `uncategorizedCount`, `enrichmentErrors`, `scopes`.
- **Compras a cuotas compartidas**: a purchase in cuotas is shared ONCE as the whole purchase (`lib/import/share-imported-rows.ts`): debts = share of `original_amount` + interés estimado (`estimateInstallmentPlan`, French annuity on the statement EA rate), keyed by `personal_debts.installment_group_id`; every cuota row carries the `split_group_id`, later cuotas attach to the existing group (no new debts). `recomputeSplitRepaid` spreads repaid across the group's rows in cuota order (`allocateRepaidAcrossRows`) so `amount − split_repaid_amount` never goes negative. `getSharedPaymentGroups` uses `group_total_amount` as the group total.
...
- **Mobile bottom-tab-bar clearance**: The mobile tab bar floats at `bottom: 0` and the brass FAB protrudes ~16px above it.
  - **Page-level scroll containers** (forms, lists, settings pages, anything that ends near the bottom) must use `MOBILE_TAB_BAR_CLEARANCE_CLASS` from `@/lib/constants/styles.ts`. The dashboard `main` already applies it, so most pages get it for free — but any nested scroll container with its own bottom padding (`pb-*`) overrides it. **Never use raw `pb-20`/`pb-24`/etc. for tab-bar clearance** — those magic numbers don't track the FAB overshoot or safe-area inset.
  - **Sheet/Drawer content** uses `MOBILE_SHEET_SAFE_AREA_CLASS` instead. Sheets and Drawers float above the tab bar (modal tier `--z-layer-modal` > nav tier `--z-layer-nav`), so their inner scroll only needs `env(safe-area-inset-bottom)` reservation, not tab-bar height.
  - **Bottom-anchored fixed bars** (e.g., `BulkActionBar`, `WizardActionBar`) anchor to `bottom: calc(var(--z-mobile-tab-bar-h) + var(--z-mobile-fab-overshoot) + env(safe-area-inset-bottom))` to sit just above the tab bar + FAB, not the raw `bottom-20`.
  - **z-index discipline**: single ascending token scale in `globals.css` (`--z-layer-*`), referenced via `z-[var(--z-layer-*)]` — never raw z-values. Order: `nav` (40, tab bar) < `modal` (1000, Dialog/AlertDialog/Sheet/Drawer/FabMenu) < `popover` (1100, Popover/Dropdown/Select/date-picker) < `toast` (1200) < `tooltip` (1300) < `dev` (9000). Popover/tooltip sit **above** modal on purpose, so a primitive opened from inside a Sheet is never hidden behind it (no per-call-site z-bump). Modal-over-modal nesting resolves via Radix DOM order at the shared `modal` tier. Never raise the tab bar above the modal tier — it breaks every overlay. Full rationale: `docs/design-system/Z_INDEX.md`.
  - **Focus mode (hide the tab bar)**: Single-task surfaces (forms, multi-step wizards, planners) hide the tab bar so users complete the task without parallel-nav distraction.
    - **Static routes**: add the path to `FOCUS_MODE_PATHS` (or a regex to `FOCUS_MODE_PATH_REGEXES`) in `webapp/src/lib/constants/mobile-nav.ts`. Currently registered: `/transactions/new`, `/recurrentes/new`, `/recurrentes/[id]/edit`, `/deudas/planificador`.
    - **Conditional/runtime hiding** (e.g., wizard step changes): call `useHideTabBar(active)` from `@/components/mobile/v2/tab-bar-visibility-provider` inside a client component. The hook is reference-counted — multiple callers compose cleanly.
    - **Required**: every focus-mode screen must render `MobileHeader variant="sub"` with a real `backHref` so the user has an explicit escape hatch. Do NOT add list/index pages to the focus-mode list — users routinely pivot away from those.
    - **Visual cue**: `FocusModeAccent` renders a 2px brass top accent automatically when the bar is hidden — no per-page wiring needed.
...
- **"Now" defaults**: Default dates/times for new records use `toColombiaDateString()` / `toColombiaTimeString()` from `@/lib/utils/date` — never `toISOString()` (UTC, rolls the day past ~7pm COT) nor `toTimeString()` (device tz).
- **Back-button discipline (history entries)**: `router.push` ONLY for destination changes (detail pages, forms, different routes). Same-view refinements — filters, search, sort, month cursor, pagination — use `router.replace`: they are view state, and pushing them makes the phone's back button replay every stale filter/month state instead of leaving the page. Overlays (sheets/drawers/FAB) that want back-to-close use the FabMenu `history.pushState` + popstate pattern and MUST consume their entry on non-back close. `onSuccess`/close side effects come from action callbacks, never from effects watching `useActionState` state — Router-Cache restores replay them on mount.
