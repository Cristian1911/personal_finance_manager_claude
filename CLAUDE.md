...
- **Mobile bottom-tab-bar clearance**: The mobile tab bar floats at `bottom: 0` and the brass FAB protrudes ~16px above it.
  ...
  - **Sheet/Drawer content** uses `MOBILE_SHEET_SAFE_AREA_CLASS` instead. Sheets (`z-[10000]`) and Drawers (`z-50`) float above the tab bar (`z-40`), so their inner scroll only needs `env(safe-area-inset-bottom)` reservation, not tab-bar height.
  ...
  - **z-index discipline**: Tab bar is `z-40`. Shadcn primitives (Dialog, AlertDialog, Drawer, Popover, Dropdown, Select, Tooltip — all `z-50`) MUST sit above the tab bar so modals correctly cover it. Never bump the tab bar above shadcn defaults — it breaks every modal interaction. `Sheet` and `FabMenu` use `z-[10000]` to stay above other modals.
