...
server.tool(
  ...
  "Obtener un resumen financiero: saldo total, disponible para gastar, pagos pendientes, deudas. Úsalo cuando el usuario pregunte '¿cómo voy?', '¿cuánto tengo?', o '¿puedo gastar X?'.",
  {
...
server.tool(
  "get_accounts",
  "Listar todas las cuentas financieras con saldos, tipos y monedas. Incluye cuentas de ahorro, corrientes, tarjetas de crédito y préstamos.",
  {},
  async () => {
...
server.tool(
  "get_transactions",
  "Buscar transacciones con filtros opcionales. Úsalo para responder '¿cuánto gasté en X?', '¿qué compré ayer?', o analizar patrones de gasto.",
  {
...
server.tool(
  "get_budget_status",
  "Ver el estado del presupuesto mensual: cuánto se asignó, cuánto se gastó y cuánto queda por categoría. Responde '¿cómo va mi presupuesto?'.",
  {
    currency: z.string().optional().describe("Código de moneda (default: COP)"),
  },
  async ({ currency }) => {
...
server.tool(
  "get_debts",
  "Ver todas las deudas: tarjetas de crédito y préstamos con saldos, tasas de interés y pagos mensuales.",
  {
    currency: z.string().optional().describe("Código de moneda (default: COP)"),
  },
  async ({ currency }) => {
...
server.tool(
  "create_transaction",
  "Registrar una transacción a partir de texto natural. Ejemplos: 'gasté 15k en café', 'me pagaron 500 mil', 'uber 8 mil ayer'. El sistema interpreta monto, tipo (gasto/ingreso) y descripción automáticamente.",
  {
