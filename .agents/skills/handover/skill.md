...
Generate a handover document at `HANDOVER.md` (project root) that gives a future Codex session full context to continue where this session left off.
...
- Environment quirks
- Patterns or conventions followed that aren't in AGENTS.md
...
- **Don't repeat AGENTS.md** — only add context that's session-specific or not already documented.
