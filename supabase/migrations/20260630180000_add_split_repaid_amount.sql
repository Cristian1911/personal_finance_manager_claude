-- ============================================================================
-- Add split_repaid_amount to transactions ("Pago compartido" — dynamic spend)
--
-- A shared payment is ONE real transaction (the full payment) plus N
-- personal_debts (lent). The transaction's EFFECTIVE spend is
--   amount - coalesce(split_repaid_amount, 0)
-- i.e. it starts at the full amount and decreases as participants repay their
-- share (each repayment recomputes split_repaid_amount = sum of repayments
-- across the split's debts). Bounded below by the user's own share, since the
-- max repayable equals the sum of the participant principals.
--
-- Non-PII plaintext passthrough (like transfer_group_id / split_group_id): no
-- encryption, no hmac, no _old routing in UPDATE. Nullable with no default, so
-- NO COALESCE preamble line is needed in the insert trigger.
-- ============================================================================

BEGIN;

ALTER TABLE public.transactions_enc
  ADD COLUMN IF NOT EXISTS split_repaid_amount numeric;

COMMENT ON COLUMN public.transactions_enc.split_repaid_amount IS
  'Total repaid so far by other participants of a shared payment (Pago compartido). Effective spend of this tx = amount - coalesce(split_repaid_amount, 0). NULL/0 for non-split transactions. Non-PII, plaintext.';

-- ============================================================================
-- Recreate transactions view + triggers to include split_repaid_amount
-- alongside all existing columns. Bodies verbatim from the current newest
-- transactions migration 20260630120000_add_split_group_id.sql, plus the new
-- plain split_repaid_amount column.
-- ============================================================================
