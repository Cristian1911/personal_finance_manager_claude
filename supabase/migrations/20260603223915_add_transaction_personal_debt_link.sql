-- ============================================================================
-- Add personal_debt_id + pd_role to transactions
--
-- Both are non-PII plaintext passthrough columns (exactly like title_locked /
-- transfer_group_id): no encryption, no hmac, no _old routing in UPDATE.
--   - personal_debt_id: FK -> personal_debts(id) ON DELETE SET NULL. Links a
--     transaction to a personal debt (origin disbursement or repayment).
--   - pd_role: enum 'origin'|'repayment'. Carries the income-exclusion +
--     repayment-recompute semantics.
--
-- 6-step _enc rebuild. Bodies copied VERBATIM from the current newest
-- transactions migration 20260525120000_add_transaction_title_locked.sql,
-- preserving the has_auth branching + SELECT INTO _old refactor. Only the two
-- new plain columns are added. Both are nullable with no default, so NO
-- COALESCE preamble line is needed (unlike title_locked).
-- personal_debts must already exist (migration 20260603210120).
-- ============================================================================
...
COMMENT ON COLUMN public.transactions_enc.personal_debt_id IS
  'FK to personal_debts. Links this transaction to a personal debt as its origin disbursement or a repayment. Non-PII, plaintext.';
COMMENT ON COLUMN public.transactions_enc.pd_role IS
  'origin = the inflow/outflow that created the debt (income/spend-excluded); repayment = normal cashflow toward the debt. Non-PII, plaintext.';

-- ============================================================================
-- Recreate transactions view + triggers to include personal_debt_id + pd_role
-- alongside all existing columns. Bodies verbatim from
-- 20260525120000_add_transaction_title_locked.sql.
-- ============================================================================
...
BEGIN
  ...
  INSERT INTO public.transactions_enc (
    ...
    NEW.personal_debt_id, NEW.pd_role,
