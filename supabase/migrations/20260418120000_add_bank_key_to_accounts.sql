-- Add bank_key TEXT to accounts (plaintext — bank identity slug for icon/parser/email routing).
-- bank_key is a lowercase slug like 'bancolombia', 'nu', 'davivienda', etc.
-- Unknown/MANUAL accounts leave this NULL → UI falls back to account_type glyph.
--
-- accounts is a view over accounts_enc, so we must:
--   1. ALTER accounts_enc
--   2. DROP VIEW CASCADE (drops triggers)
--   3. Recreate view with new column
--   4. Rebuild trigger functions (INSERT, UPDATE, DELETE)
--   5. Recreate triggers
--   6. Re-grant permissions
--   7. Backfill existing rows via regex over decrypted name

-- Step 1: Add column to real table
ALTER TABLE accounts_enc ADD COLUMN bank_key TEXT;

-- Step 2: Drop view (CASCADE drops INSTEAD OF triggers)
DROP VIEW IF EXISTS accounts CASCADE;

-- Step 3: Recreate view with bank_key as plaintext pass-through
...
-- Step 4a: Rebuild INSERT trigger function
...
-- Step 4b: Rebuild UPDATE trigger function
...
-- Step 4c: Rebuild DELETE trigger function
...
-- Step 6: Re-grant permissions
GRANT SELECT, INSERT, UPDATE, DELETE ON accounts TO authenticated;
GRANT ALL ON accounts TO postgres, service_role;

-- Step 7: Backfill existing rows via regex over decrypted name.
-- Uses zeta_decrypt_as (SECURITY DEFINER, bypasses auth.uid() check) so this
-- runs in migration context without a user session.
UPDATE accounts_enc
SET bank_key = CASE
  WHEN LOWER(zeta_decrypt_as(name, user_id)) LIKE '%bancolombia%' THEN 'bancolombia'
  WHEN LOWER(zeta_decrypt_as(name, user_id)) LIKE '%nubank%' OR LOWER(zeta_decrypt_as(name, user_id)) LIKE 'nu %' OR LOWER(zeta_decrypt_as(name, user_id)) LIKE '%nu crédito%' OR LOWER(zeta_decrypt_as(name, user_id)) LIKE '%nu tarjeta%' THEN 'nu'
  WHEN LOWER(zeta_decrypt_as(name, user_id)) LIKE '%davivienda%' THEN 'davivienda'
  WHEN LOWER(zeta_decrypt_as(name, user_id)) LIKE '%falabella%' THEN 'falabella'
  WHEN LOWER(zeta_decrypt_as(name, user_id)) LIKE '%bogot%' THEN 'banco-de-bogota'
  WHEN LOWER(zeta_decrypt_as(name, user_id)) LIKE '%lulo%' THEN 'lulo'
  WHEN LOWER(zeta_decrypt_as(name, user_id)) LIKE '%confiar%' THEN 'confiar'
  WHEN LOWER(zeta_decrypt_as(name, user_id)) LIKE '%popular%' THEN 'popular'
  WHEN LOWER(zeta_decrypt_as(name, user_id)) LIKE '%nequi%' THEN 'nequi'
  ELSE NULL
END
WHERE bank_key IS NULL;
