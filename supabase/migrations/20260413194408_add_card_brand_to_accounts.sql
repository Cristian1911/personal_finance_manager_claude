-- Add card_brand TEXT to accounts (plaintext, no encryption needed).
-- card_brand stores the card network (Visa, Mastercard, etc.) for display.
--
...
-- Step 3: Recreate view with card_brand as plaintext pass-through
CREATE VIEW accounts WITH (security_invoker = true) AS
SELECT
  account_type, available_balance, card_brand, color, connection_status,
...
-- Step 4a: Rebuild INSERT trigger function (has_auth pattern from fix migration)
...
BEGIN
  ...
  INSERT INTO accounts_enc (
    ...
    NEW.account_type, NEW.available_balance, NEW.card_brand, NEW.color,
...
-- Step 4b: Rebuild UPDATE trigger function (preserves ciphertext when has_auth=false)
...
-- Step 4c: Rebuild DELETE trigger function (unchanged, but CASCADE dropped the trigger)
...
-- Step 6: Re-grant permissions (CASCADE on the view revokes these)
