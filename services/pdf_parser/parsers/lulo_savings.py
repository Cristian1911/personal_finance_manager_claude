"""Lulo Bank Cuenta de Ahorros (savings account) PDF parser.

Parses Lulo Bank savings account statements.
...
def parse_lulo_savings(
    pdf_path: str, password: str | None = None
) -> ParsedStatement:
    """Parse Lulo Bank savings account statement."""
    ...
    with pdfplumber.open(pdf_path, password=password) as pdf:
        for page in pdf.pages:
            ...
            for line in lines:
                ...
                # TODO: Extract summary fields (saldo anterior, abonos, cargos, saldo actual)
                # TODO: Parse transaction lines
                pass

    if not transactions:
        raise ValueError(
            "No se encontraron transacciones en el extracto de Lulo Bank."
        )
