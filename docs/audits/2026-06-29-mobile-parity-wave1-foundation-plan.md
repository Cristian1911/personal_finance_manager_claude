...
- `ALTER TABLE statement_snapshots ADD COLUMN ...` ×~24 (period_from/to, currency_code, balances, credit_limit, available_credit, interest_rate, late_interest_rate, total_payment_due, minimum_payment, payment_day, ...). (P0-5b)
- `pull.ts`: register `category_rules` in SYNC_TABLES + the new snapshot columns. (`statement_snapshots`, `category_rules`, `recurring_occurrences` already in push.ts SyncTableName.)
