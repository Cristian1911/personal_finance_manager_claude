...
| AI assist (optional) | AI proposes, deterministic code decides. Jev (typed Choice/yes-no decisions with probabilities, no text generation) for picking among candidates: voice/text capture fields, notification type, near-duplicates, bills, repayments, trips, categories. Code extracts all amounts and dates. A small LLM only for naming new merchants and drafting templates. Never for idempotency keys or Disponible. See `06-ai-assist.md`. |
