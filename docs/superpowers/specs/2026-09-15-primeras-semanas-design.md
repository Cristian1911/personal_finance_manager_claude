...
| 3 | Sem 1–2 | Recurrentes | `detectRecurringCandidates`: ≥2 cobros, mismo destinatario o `cleanDescription` (+ moneda), mismo día ±3 o gap 28–34 días, cualquier `capture_method` | Existe (slice 5, `feat/recurring-candidates`): detector + chip en el detalle; el panel de Recurrentes es del slice 7 | Netflix se cobró el 5 de ago y el 5 de sep, $26.900 las dos veces. Parece un cobro mensual. ¿Lo ponemos en el calendario? (la tarjeta afirma solo las dos fechas observadas; la conclusión va con "parece") | Programar / No es recurrente | detalle de tx (chip "Parece recurrente") + panel en `/plan?tab=recurrentes` ("4 cobros parecen repetirse cada mes") |
